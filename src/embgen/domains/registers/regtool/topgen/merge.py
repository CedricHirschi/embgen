    ...
    if base_addrs is None:
            ...
            if len(block.reg_blocks) != 1:
                log.error(
                    ...
                    "device interfaces.".format(
                        instance["name"], block.name, len(block.reg_blocks)
                    )
                )
            else:
        ...
        if block_if_names != inst_if_names:
            log.error(
                ...
                "interfaces {}.".format(
                    instance["name"], inst_if_names, block.name, block_if_names
                )
            )
...
def xbar_adddevice(
    ...
    instances = [
        node for node in top["module"] + top["memory"] if node["name"] == device_base
    ]
    ...
    base_addr, size_byte = lib.get_base_and_size(name_to_block, inst, device_ifname)
    ...
    if node is None:
        log.error(
            "Cannot connect to {!r} because "
            "the crossbar defines no node for {!r}.".format(device, device_base)
        )
        return
...
def amend_xbar(
    top: Dict[str, object], name_to_block: Dict[str, IpBlock], xbar: Dict[str, object]
):
...
def amend_clocks(top: OrderedDict):
    ...
    for intf in top["exported_clks"]:
        top["inter_module"]["external"]["{}.clocks_{}".format(clkmgr_name, intf)] = (
            "clks_{}".format(intf)
        )
...
def amend_resets(top):
    ...
    for intf in top["exported_rsts"]:
        top["inter_module"]["external"]["{}.resets_{}".format(rstmgr_name, intf)] = (
            "rsts_{}".format(intf)
        )
...
def amend_reset_request(topcfg: OrderedDict, name_to_block: Dict[str, IpBlock]):
    pwrmgr_name = _find_module_name(topcfg["module"], "pwrmgr")
...
def amend_pinmux_io(top: Dict, name_to_block: Dict[str, IpBlock]):
            ...
            for sig_inst in sig_list:
                ...
                if sig_inst["width"] > 1:
                    for idx in range(sig_inst["width"]):
                        ...
                        sig_inst_copy["name"] = (
                            sig["instance"] + "_" + sig_inst_copy["name"]
                        )
    ...
    for sig in pinmux["ios"]:
        glob_idx = get_index_and_incr(
            pinmux["io_counts"], sig["connection"], sig["type"]
        )
    ...
    for sig in pinmux["ios"]:
        ...
        if sig["pad"] in known_mapped_dio_pads:
            raise SystemExit(
                "Cannot have multiple IOs mapped to the same DIO pad {}".format(
                    sig["pad"]
                )
            )
