...
from .gen_dv import gen_core_file
...
def gen_dv(top: Top, dv_base_prefix: str, outdir: str) -> int:
    """Generate DV RAL model for a Top"""
    # Read template
    lookup = TemplateLookup(directories=[str(files("topgen")), str(files("reggen"))])
    uvm_reg_tpl = lookup.get_template("top_uvm_reg.sv.tpl")

    # Expand template
    try:
        to_write = uvm_reg_tpl.render(top=top, dv_base_prefix=dv_base_prefix)
    except:  # noqa: E722
