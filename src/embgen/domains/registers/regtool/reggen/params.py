...
class LocalParam(BaseParam):
    def __init__(self, name: str, desc: Optional[str], param_type: str, value: str):
    ...
    def expand_value(self, when: str) -> int:
        try:
            ...
            raise ValueError(
                "When {}, the {} value expanded as "
                "{}, which doesn't parse as an integer.".format(
...
def _parse_parameter(where: str, raw: object) -> BaseParam:
    ...
    if name.lower().startswith("rndcnst") and "randtype" in rd:
        ...
        if r_randcount is None:
            raise ValueError(
                "At {}, the random netlist constant {} has no randcount field.".format(
                    where, name
                )
            )
    ...
    if r_default is None:
        raise ValueError(
            "At {}, the {} param has no default field.".format(where, name)
        )
    else:
        default = check_str(r_default, "default field of " + where)
        if param_type[:3] == "int":
            check_int(
                default, "default field of {}, (an integer parameter)".format(name)
            )

    if local:
        if expose:
            raise ValueError(
                "At {}, the localparam {} cannot be exposed to the top-level.".format(
                    where, name
                )
            )
...
class Params(MutableMapping):
    ...
    def apply_defaults(self, defaults: List[Tuple[str, str]]) -> None:
        for idx, (key, value) in enumerate(defaults):
            ...
            if param is None:
                raise KeyError(
                    "Cannot find parameter {} to set default value.".format(key)
                )
    ...
    def _expand_one(self, value: str, when: str) -> int:
        ...
        if param is None:
            raise ValueError(
                "Cannot find a parameter called {} when {}. "
        ...
        if not isinstance(param, LocalParam):
            raise ValueError(
                "When {}, {} is a not a local parameter.".format(when, value)
            )
    ...
    def expand(self, value: str, where: str) -> int:
        ...
        for idx, tok in enumerate(re.split(r"([+-])", value)):
            ...
            term = self._expand_one(
                tok.strip(), "expanding term {} of {}".format(idx // 2, where)
            )
