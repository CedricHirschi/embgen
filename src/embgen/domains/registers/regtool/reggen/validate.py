    ...
    if x[0] == "0" and len(x) > 2:
        if x[1] in "bB":
            ...
            if not suppress_err_msg:
                log.error(
                    err_prefix + ": int must start digit, 0b, 0B, 0o, 0O, 0x or 0X"
                )
            return 0, True
        for c in x[2:]:
            if c not in validch:
...
val_types = {
    ...
    "b": [
        "bitrange",
        "bit number as decimal integer, or bit-range as decimal integers msb:lsb",
    ],
