...
def format_comment(s: str) -> str:
    ...
    return (
        "\n".join(
            textwrap.wrap(s, width=77, initial_indent="// ", subsequent_indent="// ")
        )
        + "\n"
    )
...
def gen_cdefine_register(
    ...
    genout(
        outstr, gen_define(defname + "_REG_OFFSET", [], hex(offset), existing_defines)
    )
    ...
    for field in reg.fields:
        ...
        if field_width == 1:
            ...
            genout(
                outstr,
                gen_define(dname + "_BIT", [], str(field.bits.lsb), existing_defines),
            )
        else:
            ...
            if field_width != width:
                ...
                genout(
                    outstr, gen_define(dname + "_MASK", [], hex(mask), existing_defines)
                )
                genout(
                    outstr,
                    gen_define(
                        dname + "_OFFSET", [], str(field.bits.lsb), existing_defines
                    ),
                )
                genout(
                    outstr,
...
def gen_cdefine_window(
    ...
    items = win.items
    genout(
        outstr, gen_define(defname + "_SIZE_WORDS", [], str(items), existing_defines)
    )
    items = items * (regwidth // 8)
    genout(
        outstr, gen_define(defname + "_SIZE_BYTES", [], str(items), existing_defines)
    )
...
def gen_cdefines_module_param(
    ...
    if param.param_type not in known_types:
        warnings.warn(
            "Cannot generate a module define of type {}".format(param.param_type)
        )
        return
...
def gen_cdefine_multireg(
    ...
    for subreg in multireg.regs:
        gen_cdefine_register(
            outstr, subreg, component, regwidth, rnames, existing_defines
        )
...
def gen_cdefines_interrupt_field(
    ...
    if interrupt.bits.width() == 1:
        # single bit
        genout(
            outstr, gen_define(defname + "_BIT", [], str(fieldlsb), existing_defines)
        )
    else:
        # multiple bits (unless it is the whole register)
        if interrupt.bits.width() != regwidth:
            ...
            genout(
                outstr, gen_define(defname + "_MASK", [], hex(mask), existing_defines)
            )
            genout(
                outstr,
                gen_define(defname + "_OFFSET", [], str(fieldlsb), existing_defines),
            )
            genout(
                outstr,
                gen_define(
...
def gen_cdefines_interrupts(
    ...
    for intr in block.interrupts:
        gen_cdefines_interrupt_field(
            outstr, intr, component, regwidth, existing_defines
        )
    genout(outstr, "\n")


def gen_cdefines(
    block: IpBlock, outfile: TextIO, src_lic: Optional[str], src_copy: str
) -> int:
    ...
    gen_cdefines_module_params(
        outstr, block, block.name, block.regwidth, existing_defines
    )
...
def test_gen_define() -> None:
    ...
    assert (
        gen_define("MACRO_NAME", ["arg1", "arg2"], "arg1 + arg2", set())
        == basic_oneline_with_args
    )

    long_macro_name = (
        "A_VERY_VERY_VERY_VERY_VERY_VERY_VERY_VERY_VERY_VERY_VERY_LONG_MACRO_NAME"
    )
    ...
    assert (
        gen_define(
            long_macro_name, [], "a_fairly_long_body + something_else + 10", set()
        )
        == multiline
    )
