...
class Register(RegBase):
    ...
    def __init__(
        ...
        if self.hwext and self.hwaccess.key == "hro" and self.sw_readable():
            raise ValueError(
                "hwext flag for {} register is set, but "
                "hwaccess is hro and the register value "
                "is readable by software mode ({}).".format(
                    self.name, self.swaccess.key
                )
            )
        ...
        if self.hwext and not self.hwqe and self.sw_writable():
            raise ValueError(
                "The {} register has hwext set and is writable "
                "by software (mode {}), so must also have hwqe "
                "enabled.".format(self.name, self.swaccess.key)
            )

        self.hwre = hwre
        if self.hwre and not self.hwext:
            raise ValueError(
                "The {} register specifies hwre but not hwext.".format(self.name)
            )
        ...
        for field in self.fields:
            if field.name in self.name_to_field:
                raise ValueError(
                    "Register {} has duplicate fields called {}.".format(
    ...
    @staticmethod
    def from_raw(
        reg_width: int, offset: int, params: ReggenParams, raw: object
    ) -> "Register":
        rd = check_keys(
            raw, "register", list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys())
        )
        ...
        hwext = check_bool(
            rd.get("hwext", False), "hwext flag for {} register".format(name)
        )

        hwqe = check_bool(
            rd.get("hwqe", False), "hwqe flag for {} register".format(name)
        )

        hwre = check_bool(
            rd.get("hwre", False), "hwre flag for {} register".format(name)
        )
        ...
        if raw_resval is None:
            ...
            if not 0 <= resval < (1 << reg_width):
                raise ValueError(
                    "resval for {} register is {}, "
                    "not an unsigned {}-bit number.".format(name, resval, reg_width)
                )
        ...
        if raw_uea is None:
            ...
            update_err_alert = check_name(
                raw_uea, "update_err_alert for {} register".format(name)
            )
    ...
    def get_n_bits(self, bittype: List[str]) -> int:
        return sum(field.get_n_bits(self.hwext, bittype) for field in self.fields)
    ...
    def make_multi(
        ...
        if compact:
            ...
            new_fields = [
                field.make_suffixed(
                    "_{}".format(creg_idx), cname, creg_idx, strip_field
                )
                for field in self.fields
            ]
