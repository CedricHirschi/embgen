...
REQUIRED_FIELDS = {
    ...
    "cname": [
        "s",
        "base name for each instance, mostly"
        " useful for referring to instance in messages.",
    ],
    "fields": [
        "l",
        "list of register field description"
...
OPTIONAL_FIELDS.update(
    {
        "regwen_multi": [
            "pb",
            "If true, regwen term increments along with current multireg count.",
        ],
        "compact": [
            "pb",
            "If true, allow multireg compacting.If false, do not compact.",
        ],
    }
)
...
class MultiRegister(RegBase):
    def __init__(
        ...
        rd = check_keys(
            raw, "multireg", list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys())
        )
        ...
        # method.
        reg_allowed_keys = set(register.REQUIRED_FIELDS.keys()) | set(
            register.OPTIONAL_FIELDS.keys()
        )
        reg_rd = {key: value for key, value in rd.items() if key in reg_allowed_keys}
        self.reg = Register.from_raw(reg_width, offset, params, reg_rd)

        self.cname = check_name(
            rd["cname"], "cname field of multireg {}".format(self.reg.name)
        )
        self.name = self.reg.name

        self.regwen_multi = check_bool(
            rd.get("regwen_multi", False),
            "regwen_multi field of multireg {}".format(self.reg.name),
        )

        default_compact = True if len(self.reg.fields) == 1 else False
        self.compact = check_bool(
            rd.get("compact", default_compact),
            "compact field of multireg {}".format(self.reg.name),
        )
        if self.compact and len(self.reg.fields) > 1:
            raise ValueError(
                "Multireg {} sets the compact flag but has multiple fields.".format(
                    self.reg.name
                )
            )

        count_str = check_str(
            rd["count"], "count field of multireg {}".format(self.reg.name)
        )
        self.count = params.expand(
            count_str, "count field of multireg " + self.reg.name
        )
        if self.count <= 0:
            raise ValueError(
                "Multireg {} has a count of {}, which isn't positive.".format(
                    self.reg.name, self.count
                )
            )
