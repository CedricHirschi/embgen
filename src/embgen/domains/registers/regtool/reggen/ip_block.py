...
REQUIRED_FIELDS = {
    ...
    "registers": ["l", "list of register definition groups and offset control groups"],
}
...
OPTIONAL_FIELDS = {
    ...
    "hier_path": [None, "additional hierarchy path before the reg block instance"],
    "interrupt_list": ["lnw", "list of peripheral interrupts"],
    "inter_signal_list": ["l", "list of inter-module signals"],
    "no_auto_alert_regs": [
        "s",
        "Set to true to suppress automatic "
    ...
    "SPDX-License-Identifier": [
        "s",
        "License ientifier (if using pure json) "
...
class IpBlock:
    ...
    @staticmethod
    def from_raw(
        param_defaults: List[Tuple[str, str]], raw: object, where: str
    ) -> "IpBlock":
        rd = check_keys(
            raw,
            "block at " + where,
        ...
        if r_regwidth is None:
            ...
            if regwidth <= 0:
                raise ValueError(
                    "Invalid regwidth field for {}: {} is not positive.".format(
                        what, regwidth
                    )
                )

        params = ReggenParams.from_raw(
            "parameter list for " + what, rd.get("param_list", [])
        )
        try:
            params.apply_defaults(param_defaults)
        except (ValueError, KeyError) as err:
            raise ValueError(
                "Failed to apply defaults to params: {}".format(err)
            ) from None

        init_block = RegBlock(regwidth, params)

        interrupts = Signal.from_raw_list(
            "interrupt_list for block {}".format(name), rd.get("interrupt_list", [])
        )
        alerts = Alert.from_raw_list(
            "alert_list for block {}".format(name), rd.get("alert_list", [])
        )

        no_auto_intr = check_bool(
            rd.get("no_auto_intr_regs", not interrupts),
            "no_auto_intr_regs field of " + what,
        )

        no_auto_alert = check_bool(
            rd.get("no_auto_alert_regs", not alerts),
        ...
        if interrupts and not no_auto_intr:
            if interrupts[-1].bits.msb >= regwidth:
                raise ValueError(
                    "Interrupt list for {} is too wide: "
                    "msb is {}, which doesn't fit with a "
                    "regwidth of {}.".format(what, interrupts[-1].bits.msb, regwidth)
                )
        ...
        if alerts:
            ...
            if existing_param is not None:
                if (
                    not isinstance(existing_param, LocalParam)
                    or existing_param.param_type != "int"
                    or existing_param.value != str(len(alerts))
                ):
                    raise ValueError("Conflicting definition of NumAlerts parameter.")
            else:
                params.add(
                    LocalParam(
                        name="NumAlerts",
        ...
        r_inter_signals = check_list(
            rd.get("inter_signal_list", []), "inter_signal_list field"
        )
        inter_signals = [
            InterSignal.from_raw(
                "entry {} of the inter_signal_list field".format(idx + 1), entry
            )
        ...
        hier_path = check_optional_str(
            rd.get("hier_path", None), "hier_path field of " + what
        )

        clock_primary = check_name(
            rd["clock_primary"], "clock_primary field of " + what
        )
        other_clock_list = check_name_list(
            rd.get("other_clock_list", []), "other_clock_list field of " + what
        )
        clock_signals = [clock_primary] + other_clock_list

        reset_primary = check_name(
            rd.get("reset_primary", "rst_ni"), "reset_primary field of " + what
        )
        other_reset_list = check_name_list(
            rd.get("other_reset_list", []), "other_reset_list field of " + what
        )
        reset_signals = [reset_primary] + other_reset_list

        xputs = (
            Signal.from_raw_list(
                "available_inout_list for block " + name,
                rd.get("available_inout_list", []),
            ),
            Signal.from_raw_list(
                "available_input_list for block " + name,
        ...
        wakeups = Signal.from_raw_list(
            "wakeup_list for block " + name, rd.get("wakeup_list", [])
        )
        rst_reqs = Signal.from_raw_list(
            "reset_request_list for block " + name, rd.get("reset_request_list", [])
        )

        scan_reset = check_bool(
            rd.get("scan_reset", False), "scan_reset field of " + what
        )

        scan_en = check_bool(rd.get("scan_en", False), "scan_en field of " + what)
        ...
        if set(reg_block_names) != set(dev_if_names):
            raise ValueError(
                "IP block {} defines device interfaces, named {} "
                "but its registers don't match (they are keyed "
                "by {}).".format(name, dev_if_names, list(reg_block_names))
            )
    ...
    @staticmethod
    def from_text(
        txt: str, param_defaults: List[Tuple[str, str]], where: str
    ) -> "IpBlock":
        """Load an IpBlock from an hjson description in txt"""
        return IpBlock.from_raw(
            param_defaults, hjson.loads(txt, use_decimal=True), where
        )

    @staticmethod
    def from_path(path: str, param_defaults: List[Tuple[str, str]]) -> "IpBlock":
        """Load an IpBlock from an hjson description in a file at path"""
        with open(path, "r", encoding="utf-8") as handle:
            return IpBlock.from_text(
                handle.read(), param_defaults, "file at {!r}".format(path)
            )
    ...
    def _asdict(self) -> Dict[str, object]:
        ...
        if len(self.reg_blocks) == 1 and None in self.reg_blocks:
            ...
            ret["registers"] = {k: v.as_dicts() for k, v in self.reg_blocks.items()}
    ...
    def get_signal_by_name_as_dict(self, name: str) -> Dict:
        ...
        for sig in sig_list:
            ...
            raise ValueError(
                "Signal {} does not exist in IP block {}".format(name, self.name)
            )
