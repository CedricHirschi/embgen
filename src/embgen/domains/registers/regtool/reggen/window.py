...
OPTIONAL_FIELDS = {
    "data-intg-passthru": [
        "s",
        "True if the window has data integrity pass through. "
        "Defaults to false if not present.",
    ],
    "byte-write": [
        "s",
        "True if byte writes are supported. Defaults to false if not present.",
    ],
    "validbits": [
        "d",
...
class Window:
    ...
    @staticmethod
    def from_raw(
        offset: int, reg_width: int, params: ReggenParams, raw: object
    ) -> "Window":
        rd = check_keys(
            raw, "window", list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys())
        )
        ...
        byte_write = check_bool(
            rd.get("byte-write", False), "byte-write field for " + wind_desc
        )
        ...
        validbits = check_int(
            rd.get("validbits", reg_width), "validbits field for " + wind_desc
        )
        if validbits <= 0:
            raise ValueError(
                "validbits field for {} is not positive.".format(wind_desc)
            )
        ...
        if items <= 0:
            raise ValueError(
                "Items field for {} is {}, which isn't positive.".format(
                    wind_desc, items
                )
            )
        ...
        if po2_size != size_in_bytes and not unusual:
            raise ValueError(
                ...
                'set the "unusual" flag.'.format(
                    wind_desc, items, size_in_bytes, po2_size
                )
            )
