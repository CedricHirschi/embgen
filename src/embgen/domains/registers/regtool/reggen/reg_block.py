...
class RegBlock:
    ...
    @staticmethod
    def build_blocks(block: "RegBlock", raw: object) -> Dict[Optional[str], "RegBlock"]:
        ...
        if not isinstance(raw, dict):
            raise ValueError(
                "registers field at top-level is neither a list or a dictionary."
            )
        ...
        for idx, (r_key, r_val) in enumerate(raw.items()):
            ...
            rb_key = check_str(
                r_key,
                "the key for item {} of the registers dictionary at top-level".format(
                    idx + 1
                ),
            )
            rb_val = check_list(
                r_val,
                "the value for item {} of the registers dictionary at top-level".format(
                    idx + 1
                ),
            )
            ...
            block.add_raw_registers(
                rb_val,
                "item {} of the registers dictionary at top-level".format(idx + 1),
            )
            block.validate()
    ...
    def add_raw_registers(self, raw: object, what: str) -> None:
        ...
        for entry_idx, entry_raw in enumerate(rl):
            where = "entry {} of the top-level registers field".format(entry_idx + 1)
        ...
        for t in ["reserved", "skipto", "window", "multireg"]:
            ...
            if t_body is not None:
                ...
                if len(entry) != 1:
                    ...
                    assert other_keys
                    raise ValueError(
                        "At offset {:#x}, {} has key {}, which "
                        "should give its type. But it also has "
                        "other keys too: {}.".format(
                            self.offset, where, t, ", ".join(other_keys)
                        )
                    )
                entry_type = t
                entry_body = t_body
    ...
    def _handle_reserved(self, where: str, body: object) -> None:
        ...
        if nreserved <= 0:
            raise ValueError(
                "Reserved count in {} is {}, which is not positive.".format(
                    where, nreserved
                )
            )
    ...
    def _handle_skipto(self, where: str, body: object) -> None:
        ...
        if skipto % self._addrsep:
            raise ValueError(
                "Destination of skipto in {} is {:#x}, "
                "not a multiple of addrsep, {:#x}.".format(where, skipto, self._addrsep)
            )
        self.offset = skipto

    def _handle_window(self, where: str, body: object) -> None:
        window = Window.from_raw(self.offset, self._reg_width, self._params, body)
            ...
            if lname in self.name_to_offset:
                raise ValueError(
                    ...
                    "same name as something at offset {:#x}.".format(
                        window.name, window.offset, self.name_to_offset[lname]
                    )
                )
    ...
    def _handle_multireg(self, where: str, body: object) -> None:
        mr = MultiRegister(
            self.offset, self._addrsep, self._reg_width, self._params, body
        )
        for reg in mr.regs:
            lname = reg.name.lower()
            if lname in self.name_to_offset:
                raise ValueError(
                    "Multiregister {} (at offset {:#x}) expands "
    ...
    def add_register(self, reg: Register) -> None:
        ...
        if lname in self.name_to_offset:
            raise ValueError(
                "Register {} (at offset {:#x}) has the same "
                "name as something at offset {:#x}.".format(
                    reg.name, reg.offset, self.name_to_offset[lname]
                )
            )
        self._add_flat_reg(reg)
        self.name_to_offset[lname] = reg.offset
    ...
    def validate(self) -> None:
        ...
        for wenname in self.wennames:
            ...
            if re.fullmatch(r"(.+_)*REGWEN(_[0-9]+)?", wenname) is None:
                raise ValueError(
                    "Regwen name {} must have the suffix '_REGWEN'".format(wenname)
                )
            ...
            if wen_reg is None:
                raise ValueError(
                    ...
                    "write-enable, but there is no such register.".format(wenname)
                )
            ...
            if wen_reg.swaccess.key != "ro" and not wen_reg.resval:
                raise ValueError(
                    ...
                    "it should have a nonzero reset value.".format(wenname)
                )
            ...
            raise ValueError(
                ...
                "or have swaccess=RO and hwaccess=HWO.".format(
                    wenname, wen_reg.swaccess.key, wen_reg.hwaccess.key
                )
            )
    ...
    def as_dicts(self) -> List[object]:
        ...
        for entry in self.entries:
            assert (
                isinstance(entry, Register)
                or isinstance(entry, MultiRegister)
                or isinstance(entry, Window)
            )
    ...
    def make_intr_regs(self, interrupts: Sequence[Signal]) -> None:
        ...
        self._add_intr_alert_reg(
            ...
            lambda w, n: (
                "Enable interrupt when {}!!INTR_STATE.{} is set.".format(
        ...
        self._add_intr_alert_reg(
            ...
            lambda w, n: (
                "Write 1 to force {}!!INTR_STATE.{} to 1.".format(
                    "corresponding bit in " if w else "", n
                )
            ),
            "wo",
