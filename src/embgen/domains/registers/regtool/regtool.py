...
def main():
    ...
    parser.add_argument(
        "-d", action="store_true", help="Output register documentation (markdown)"
    )
    parser.add_argument(
        "--cdefines", "-D", action="store_true", help="Output C defines header"
    )
    parser.add_argument(
        "--doc-html-old",
        action="store_true",
    ...
    parser.add_argument(
        "-f", action="store_true", help="Output as FPV CSR rw assertion module"
    )
    parser.add_argument(
        "--outdir",
        "-t",
        help="Target directory for generated RTL; tool uses ../rtl if blank.",
    )
    parser.add_argument(
        "--dv-base-prefix",
        default="dv_base",
    ...
    parser.add_argument(
        "--novalidate", action="store_true", help="Skip validate, just output json"
    )
    ...
    for arg_name, spec in arg_to_format:
        if getattr(args, arg_name):
            if format is not None:
                log.error(
                    "Multiple output formats specified on "
    ...
    if dirspec is None:
        if args.outdir is not None:
            log.error(
                "The {} format expects an output file, not an output directory.".format(
                    format
                )
            )
            sys.exit(1)
        ...
        if args.outfile is not sys.stdout:
            log.error(
                "The {} format expects an output directory, not an output file.".format(
                    format
                )
            )
            sys.exit(1)
        ...
        if args.outdir is not None:
            ...
            log.error(
                ...
                "from stdin. Use --outdir to specify it manually.".format(format)
            )
            sys.exit(1)
