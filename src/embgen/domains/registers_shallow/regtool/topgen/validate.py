...
top_required = {
    ...
    "debug_mem_base_addr": ["d", "Base address of RV_DM. Planned to move to module"],
...
top_optional = {
    ...
    "alert_module": ["l", "list of the modules that connects to alert_handler"],
...
pinmux_sig_required = {
    ...
    "connection": [
        "s",
        "Specification of connection type, can be direct, manual or muxed",
    ],
}
pinmux_sig_optional = {
...
class Flash:
    ...
    def check_values(self):
        pow2_check = (
            self.is_pow2(self.banks)
            and self.is_pow2(self.pages_per_bank)
            and self.is_pow2(self.program_resolution)
        )
...
def check_pad(
    ...
    if pad["connection"] not in valid_connections:
        log.warning(
            "Connection type {} of pad {} is invalid".format(
...
def check_pinout(top: Dict, prefix: str) -> int:
    ...
    for pad in top["pinout"]["pads"]:
        error += check_keys(
            pad, pad_required, pad_optional, pad_added, prefix + " Pinout"
        )

        error += check_pad(
            top, pad, known_names, ["direct", "manual", "muxed"], prefix + " Pad"
        )
...
def check_pinmux(top: Dict, prefix: str) -> int:
    ...
    for sig in top["pinmux"]["signals"]:
        ...
        if padname != "":
            if padname in known_direct_pads:
                if known_direct_pads[padname] == 1:
                    ...
                    log.warning(
                        "Warning, direct pad {} is already connected".format(padname)
                    )
                    error += 1
            else:
        ...
        if sig["connection"] == "direct":
            ...
            if padname != "":
                log.warning(
                    "Manual connection of instance {} port {} "
                    "must not have an associated pad".format(
                        sig["instance"], sig["port"]
                    )
                )
...
def check_implementation_targets(top: Dict, prefix: str) -> int:
    ...
    for target in top["targets"]:
        error += check_keys(
            target, target_required, target_optional, target_added, prefix + " Targets"
        )
        ...
        for pad in target["pinout"]["add_pads"]:
            error += check_pad(
                top, pad, known_pad_names, ["manual"], prefix + " Additional Pad"
            )
...
def check_clock_groups(top):
    ...
    for group in top["clocks"]["groups"]:
        ...
        if group["src"] == "yes" and group["sw_cg"] != "no":
            log.error(
                "Invalid combination of src and sw_cg: {} and {}".format(
...
def check_clocks_resets(top, ipobjs, ip_idxs, xbarobjs, xbar_idxs):
    ...
    clock_srcs = [
        clock["name"] for clock in top["clocks"]["srcs"] + top["clocks"]["derived_srcs"]
    ]
    ...
    for xbarcfg in top["xbar"]:
        ...
        error += validate_reset(
            xbarcfg, xbarobjs[xbar_idxs[xbarcfg_name]], reset_nets, "xbar"
        )
        error += validate_clock(
            xbarcfg, xbarobjs[xbar_idxs[xbarcfg_name]], clock_srcs, "xbar"
        )
...
def validate_reset(top, inst, reset_nets, prefix=""):
    ...
    if isinstance(inst, IpBlock):
        ...
        name = inst["name"]
        reset_signals = [inst.get("reset_primary", "rst_ni")] + inst.get(
            "other_reset_list", []
        )
    ...
    if len(top["reset_connections"]) != len(reset_signals):
        error += 1
        log.error("%s %s mismatched number of reset ports and nets" % (prefix, name))

    missing_port = [
        port for port in top["reset_connections"].keys() if port not in reset_signals
    ]
    ...
    missing_net = [
        net for port, net in top["reset_connections"].items() if net not in reset_nets
    ]
...
def validate_clock(top, inst, clock_srcs, prefix=""):
    ...
    if isinstance(inst, IpBlock):
        ...
        name = inst["name"]
        clock_signals = [inst.get("clock_primary", "rst_ni")] + inst.get(
            "other_clock_list", []
        )

    if len(top["clock_srcs"]) != len(clock_signals):
        error += 1
        log.error("%s %s mismatched number of clock ports and nets" % (prefix, name))
    ...
    if missing_port:
        error += 1
        log.error("%s %s Following clock ports do not exist:" % (prefix, name))
        [log.error("%s" % port) for port in missing_port]
    ...
    if missing_net:
        error += 1
        log.error("%s %s Following clock nets do not exist:" % (prefix, name))
        [log.error("%s" % net) for net in missing_net]
...
def check_flash(top):
    ...
    for mem in top["memory"]:
        if mem["type"] == "eflash":
            error = check_keys(
                mem, eflash_required, eflash_optional, eflash_added, "Eflash"
            )
            ...
            if "domains" not in reset:
                ...
                for domain in reset["domains"]:
                    if domain not in top["power"]["domains"]:
                        log.error(
                            "{} defined invalid domain {}".format(reset["name"], domain)
                        )
                        error += 1
                        return error
