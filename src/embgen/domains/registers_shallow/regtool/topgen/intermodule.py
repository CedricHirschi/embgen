...
def add_intermodule_connection(
    obj: OrderedDict, req_m: str, req_s: str, rsp_m: str, rsp_s: str
):
...
def elab_intermodule(topcfg: OrderedDict):
    ...
    for req, rsps in topcfg["inter_module"]["connect"].items():
        ...
        req_struct = find_intermodule_signal(
            list_of_intersignals, req_module, req_signal
        )
        ...
        if "package" in req_struct:
            ...
            for rsp in rsps:
                ...
                rsp_struct = find_intermodule_signal(
                    list_of_intersignals, rsp_module, rsp_signal
                )
        ...
        for i, rsp in enumerate(rsps):
            ...
            if req_struct["struct"] == "logic":
                assert req_struct["type"] != "req_rsp", (
                    "logic signal cannot have req_rsp type"
                )
...
# Validation
def check_intermodule_field(
    sig: OrderedDict, prefix: str = ""
    ...
    if "width" not in sig:
        ...
        if err:
            log.error(
                "{prefix} Inter-module {inst}.{sig} 'width' should be int type.".format(
                    prefix=prefix, inst=sig["inst_name"], sig=sig["name"]
                )
            )
            error += 1
        else:
...
def check_intermodule(topcfg: Dict, prefix: str) -> int:
    ...
    for req, rsps in topcfg["inter_module"]["connect"].items():
        ...
        if req_s == "":
            log.error(
                "Cannot parse the inter-module signal key '{req}'".format(req=req)
            )
            error += 1
        ...
        req_struct = find_intermodule_signal(
            topcfg["inter_signal"]["signals"], req_m, req_s
        )
        ...
        if req_i != -1 and len(rsps) != 1:
            ...
            log.error(
                "If key {req} has index, only one entry is allowed.".format(req=req)
            )
            error += 1
        ...
        if req_struct["width"] > 1 or len(rsps) != 1:
            ...
            # If not, error
                ...
                log.error(
                    "'uni' type connection {req} should be either "
    ...
    for item in topcfg["inter_module"]["top"] + list(
        ...
        sig_struct = find_intermodule_signal(
            topcfg["inter_signal"]["signals"], sig_m, sig_s
        )
...
def get_dangling_im_def(objs: OrderedDict) -> str:
    ...
    """
    unused_def = [
        obj for obj in objs if obj["end_idx"] > 0 and obj["act"] == obj["suffix"]
    ]
