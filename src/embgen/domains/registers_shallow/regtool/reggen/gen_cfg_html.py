...
def gen_cfg_html(cfgs: IpBlock, outfile: TextIO) -> None:
    ...
    # clocks
    gen_kv(
        outfile, "Primary Clock", "<b><code>{}</code></b>".format(cfgs.clock_signals[0])
    )
    if len(cfgs.clock_signals) > 1:
        other_clocks = [
            "<b><code>{}</code></b>".format(clk) for clk in cfgs.clock_signals[1:]
        ]
        ...
        for direction, x in ios:
            genout(
                ...
                "<tr><td>{}</td><td>{}</td>{}</tr>".format(
                    name_width(x), direction, render_td(x.desc, rnames, None)
                ),
            )
    ...
    if not cfgs.interrupts:
        ...
        genout(
            outfile,
            '<table class="cfgtable"><tr><th>Interrupt Name</th>'
            + "<th>Description</th></tr>\n",
        )
        for x in cfgs.interrupts:
            genout(
                outfile,
                "<tr><td>{}</td>{}</tr>".format(
                    name_width(x), render_td(x.desc, rnames, None)
                ),
            )
        genout(outfile, "</table>\n")
    ...
    if not cfgs.alerts:
        ...
        genout(
            outfile,
            '<table class="cfgtable"><tr><th>Alert Name</th>'
            + "<th>Description</th></tr>\n",
        )
