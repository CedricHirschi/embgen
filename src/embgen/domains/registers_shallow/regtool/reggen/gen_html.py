...
def gen_html_reg_pic(outfile: TextIO, reg: Register, width: int) -> None:
    ...
    for field in reversed(reg.fields):
        ...
        while nextbit > fieldmsb:
            ...
            genout(
                outfile, '<td class="unused" colspan=' + str(spans) + ">&nbsp;</td>\n"
            )
            if (nextbit >= nextline) and (fieldmsb < nextline):
                nextbit = nextline - 1
        ...
        while (fieldmsb >= nextline) and (fieldlsb < nextline):
            ...
            genout(
                outfile,
                '<td class="fname" colspan=' + str(spans) + ">" + fname + "...</td>\n",
            )
            fname = "..." + field.name
        ...
        if namelen > bsize * fieldwidth:
            usestyle = (
                ' style="font-size:' + str((bsize * 100 * fieldwidth) / namelen) + '%"'
            )
        else:
            usestyle = ""
...
def gen_html_register(
    outfile: TextIO, reg: Register, comp: str, width: int, rnames: Set[str]
) -> None:
    rname = reg.name
    offset = reg.offset
    regwen_div = ""
    ...
    if desc_body:
        genout(outfile, "<tr><td colspan=5>{}</td></tr>".format("".join(desc_body)))
    ...
    for field in reg.fields:
        ...
        if fieldlsb > nextbit:
            ...
            genout(outfile, "</td><td></td><td></td><td></td><td>Reserved</td></tr>")
        ...
        if field.enum is not None:
            ...
            for enum in field.enum:
                ...
                desc_parts.append(
                    "<tr><td>{val}</td><td>{name}</td><td>{desc}</td></tr>\n".format(
                        val=enum.value, name=enum.name, desc="".join(enum_desc_paras)
                    )
                )
            desc_parts.append("</table>")
        ...
        genout(outfile, '<td class="regde">{}</td>'.format("".join(desc_parts)))
...
def gen_html_window(
    outfile: TextIO, win: Window, comp: str, regwidth: int, rnames: Set[str]
) -> None:
    ...
    for x in [0, 1, 2, tblmax - 1, tblmax]:
        if x == 2:
            genout(
                outfile,
                "<tr><td>&nbsp;</td><td align=center colspan="
                + str(regwidth)
                + ">...</td></tr>",
            )
        else:
            ...
            if wid < regwidth:
                genout(
                    outfile,
                    '<td class="unused" colspan='
                    + str(regwidth - wid)
                    + ">&nbsp;</td>\n",
                )
                genout(
                    outfile, '<td class="fname" colspan=' + str(wid) + ">&nbsp;</td>\n"
                )
            else:
                genout(
                    outfile,
                    '<td class="fname" colspan=' + str(regwidth) + ">&nbsp;</td>\n",
                )
            genout(outfile, "</tr>")
...
def gen_html_reg_block(
    outfile: TextIO, rb: RegBlock, comp: str, width: int, rnames: Set[str]
) -> None:
    for x in rb.entries:
...
def gen_html(block: IpBlock, outfile: TextIO) -> int:
    ...
    for iface_name, rb in block.reg_blocks.items():
        iface_desc = (
            "device interface <code>{}</code>".format(iface_name)
            if iface_name is not None
            else "the unnamed device interface"
        )
        genout(outfile, "<h3>Registers visible under {}</h3>".format(iface_desc))
        gen_html_reg_block(outfile, rb, block.name, block.regwidth, rnames)
