...
class Field:
    ...
    def from_raw(
        ...
        rd = check_keys(
            raw, where, list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys())
        )

        raw_name = rd.get("name")
        if raw_name is None:
            name = "field{}".format(field_idx + 1) if num_fields > 1 else reg_name
        else:
            name = check_name(raw_name, "name of {}".format(where))

        raw_desc = rd.get("desc")
        if raw_desc is None and raw_name is not None:
            raise ValueError("Missing desc field for {}".format(where))
        if raw_desc is None:
            desc = None
        else:
            desc = check_str(raw_desc, "desc field for {}".format(where))

        tags = check_str_list(rd.get("tags", []), "tags for {}".format(where))
        ...
        if raw_resval is None:
            ...
            if resval is None:
                ...
                if reg_resval is not None:
                    raise ValueError(
                        'resval field for {} is "x", but the '
                        "register defines a resval as well.".format(where)
                    )
            else:
                # Check that the reset value is representable with bits
                if not (0 <= resval <= bits.max_value()):
                    raise ValueError(
                        "resval field for {} is {}, which "
                        "isn't representable as an unsigned "
                        "{}-bit integer.".format(where, resval, bits.width())
                    )
                ...
                if reg_resval is not None:
                    ...
                    if resval != resval_from_reg:
                        raise ValueError(
                            "resval field for {} is {}, but the "
                            "register defines a resval as well, "
                            "where bits {}:{} would give {}.".format(
                                where, resval, bits.msb, bits.lsb, resval_from_reg
                            )
                        )
        ...
        if raw_enum is None:
            ...
            enum = []
            raw_entries = check_list(raw_enum, "enum field for {}".format(where))
            enum_val_to_name = {}  # type: Dict[int, str]
            for idx, raw_entry in enumerate(raw_entries):
                entry = EnumEntry(
                    "entry {} in enum list for {}".format(idx + 1, where),
                    bits.max_value(),
                    raw_entry,
                )
                if entry.value in enum_val_to_name:
                    raise ValueError(
                        "In {}, duplicate enum entries for "
                        "value {} ({} and {}).".format(
                            where,
                            entry.value,
        ...
        return Field(
            name, desc, tags, swaccess, hwaccess, reg_hwqe, reg_hwre, bits, resval, enum
        )

    def has_incomplete_enum(self) -> bool:
        return self.enum is not None and len(self.enum) != 1 + self.bits.max_value()
    ...
    def make_multi(
        ...
        if field_width * num_copies > reg_width:
            raise ValueError(
                "Cannot replicate field {} {} times: the "
                "resulting width would be {}, but the register "
                "width is just {}.".format(
                    self.name, num_copies, field_width * num_copies, reg_width
                )
            )

        desc = "For {}{}".format(cname, creg_idx) if stripped else self.desc
        ...
        for reg_idx in range(min_reg_idx, max_reg_idx + 1):
            ...
            bits = (
                self.bits if bit_offset == 0 else self.bits.make_translated(bit_offset)
            )
    ...
    def make_suffixed(
        self, suffix: str, cname: str, creg_idx: int, stripped: bool
    ) -> "Field":
        desc = "For {}{}".format(cname, creg_idx) if stripped else self.desc
        enum = None if stripped else self.enum
