...
def gen_cfg_md(
    cfgs: IpBlock, output: TextIO, register_file: Optional[str] = None
) -> None:
    comport_url = url(
    ...
    list_items.append(
        "Other Clocks: "
        + (
            ", ".join(coderef(clk) for clk in other_clocks)
            if other_clocks
            else italic("none")
        )
    )
    ...
    # Bus Interfaces
    dev_ports = [
        coderef(port) for port in cfgs.bus_interfaces.get_port_names(False, True)
    ]
    assert dev_ports
    list_items.append("Bus Device Interfaces (TL-UL): " + ", ".join(dev_ports))

    host_ports = [
        coderef(port) for port in cfgs.bus_interfaces.get_port_names(True, False)
    ]
    list_items.append(
    ...
    if not cfgs.inter_signals:
        ...
        for ims in cfgs.inter_signals:
            name = ims.name
            pkg_struct = (
                ims.package + "::" + ims.struct
                if ims.package is not None
                else ims.struct
            )
