...
class BusInterfaces:
    ...
    def from_raw(raw: object, where: str) -> "BusInterfaces":
        ...
        for idx, raw_entry in enumerate(check_list(raw, where)):
            ...
            ed = check_keys(raw_entry, entry_what, ["protocol", "direction"], ["name"])

            protocol = check_str(ed["protocol"], "protocol field of " + entry_what)
            if not BusProtocol.has_value(protocol):
                raise ValueError(
                    "Unknown protocol {!r} at {}".format(protocol, entry_what)
                )

            direction = check_str(ed["direction"], "direction field of " + entry_what)
            if direction not in ["device", "host"]:
                raise ValueError(
                    "Unknown interface direction {!r} at {}".format(
                        direction, entry_what
                    )
                )

            name = check_optional_str(ed.get("name"), "name field of " + entry_what)

            if direction == "host":
                if name is None:
                    if has_unnamed_host:
                        raise ValueError(
                            "Multiple un-named host interfaces at {}".format(where)
                        )
                    has_unnamed_host = True
                else:
                    if name in named_hosts:
                        raise ValueError(
                            "Duplicate host interface with name {!r} at {}".format(
                                name, where
                            )
                        )
                ...
                if name is None:
                    if has_unnamed_device:
                        raise ValueError(
                            "Multiple un-named device interfaces at {}".format(where)
                        )
                    has_unnamed_device = True
                else:
                    if name in named_devices:
                        raise ValueError(
                            "Duplicate device interface with name {!r} at {}".format(
                                name, where
                            )
                        )
    ...
    def get_port_name(self, is_host: bool, name: Optional[str]) -> str:
        ...
        return tl_suffix if name is None else "{}_{}".format(name, tl_suffix)
    ...
    def _if_inter_signal(self, is_host: bool, name: Optional[str]) -> InterSignal:
        return InterSignal(
            self.get_port_name(is_host, name),
            None,
            "tl",
            "tlul_pkg",
            "req_rsp",
    ...
    def inter_signals(self) -> List[InterSignal]:
        return [
            self._if_inter_signal(is_host, name) for is_host, name in self._interfaces()
        ]
    ...
    def find_port_name(self, is_host: bool, name: Optional[str]) -> str:
        ...
        if not self.has_interface(is_host, name):
            called = "with no name" if name is None else "called {!r}".format(name)
            raise KeyError(
                "There is no {} bus interface {}.".format(
                    "host" if is_host else "device", called
                )
            )
