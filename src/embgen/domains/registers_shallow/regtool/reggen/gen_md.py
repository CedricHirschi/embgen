...
def gen_md_register_summary(
    ...
    def add_row(
        name: str, anchor: str, offset: int, length: int, description: str
    ) -> None:
        rows.append(
            [
...
def gen_md_multiregister(
    output: TextIO, mreg: MultiRegister, comp: str, width: int
) -> None:
...
def gen_md_register(output: TextIO, reg: Register, comp: str, width: int) -> None:
    ...
    if reg.regwen is not None:
        output.write(
            list_item(
                "Register enable: " + url(mono(reg.regwen), "#" + reg.regwen.lower())
            )
        )
...
def gen_md_reg_fields(output: TextIO, reg: Register, width: int) -> None:
    ...
    field_sections = any(
        ...
        or (field.desc is not None and len(field.desc) > MAX_DESCRIPTION_LEN)
        for field in reg.fields
    )
